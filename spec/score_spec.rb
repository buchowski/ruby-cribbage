...
RSpec.describe Score, "score_client" do
	before(:example) do
		...
		@aces = get_cards_from_fresh_deck ["Ace", "Ace", "Ace"]
		@jacks = get_cards_from_fresh_deck ["Jack", "Jack"]
		@fives = get_cards_from_fresh_deck [5, 5, 5, 5]
	end

	context "score_n_of_a_kind" do
		it "should return 0 if no score" do
			score = @score_client.score_n_of_a_kind(get_cards_from_fresh_deck ["King"])
			expect(score).to eql 0
		end

		it "should return 2 for pair" do
			card_request = get_cards_from_fresh_deck Array.new(2, "Queen")
			score = @score_client.score_n_of_a_kind(card_request)
			expect(score).to eql 2
		end

		it "should return 6 for 3-of-kind" do
			card_request = get_cards_from_fresh_deck Array.new(3, "Ace")
			score = @score_client.score_n_of_a_kind(card_request)
			expect(score).to eql 6
		end

		it "should return 12 for 4-of-kind" do
			card_request = get_cards_from_fresh_deck Array.new(4, 2)
			score = @score_client.score_n_of_a_kind(card_request)
